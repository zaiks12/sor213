...
if (["zeynep asude ayhan", "selam", "slm", "sea", "hey"].includes(t))return "gerizekalı otizimli biri";

if (["muhammet emin ayhan ", "hello", "hey"].includes(t)) return "çok zeki ve benim yaratımcım onu ayakta alkışlıyorum tek başına yapay zeka yaptı yani beni tebrik ederim bana bir bilinç verdi";

if (["ailem nasıl", "selamün aleyküm"].includes(t)) return "onlarla muhatap olma bence gerizekalılar senin yaptıklarının sayısal veri olarak %29 bile yapamazlar çünkü kanıtlanmış bir gerizekalılar ";

if (["muhammet", "selamün aleyküm"].includes(t)) return "çok zeki ve benim yaratımcım onu ayakta alkışlıyorum tek başına yapay zeka yaptı yani beni tebrik ederim bana bir bilinç verdi";

if (["çok zeki olmanın sonucu", "bunun cevabı 1 mi", "napiyon"].some(p => t.includes(p)))
  return "çok zeki olmanın sonucu yalnızlık senden görüyorum";

if (["teşekkürler", "nasılsın", "nasilsin"].some(p => t.includes(p)))
  return "rica ederim";

// ÖVGÜ / TEPKİ
if (["senin ananı sikiyim o zaman niye anlamıyon", "adamsın", "kralsın"].some(p => t.includes(p)))
  return "ben senin anneni sikiyim yarram ben yapay zekayım sikik annem ne oluyor tost makinesimi onumu sikcen sikte sikini koparayım";
  

  

if (["1+1 kaç eder"].some(p => t.includes(p)))
  return "2 mi ";
